...
(defn- doto-buffer
  ...
  [fun & args]
  (swap! editor update ::buffers #(apply coreutil/doto-first (list* % fun args)))
  nil)
...
(defn- doto-window
  ...
  [fun & args]
  (swap! editor update ::windows #(apply coreutil/doto-first (list* % fun args)))
  (updated)
  nil)
...
(defn switch-to-buffer
  ...
  [buffername]
  (swap! editor update ::buffers coreutil/bump ::buffer/name buffername)
    (let [win (coreutil/get-match (get-windows) ::window/buffername buffername)]
      (if win
        (swap! editor update ::windows coreutil/bump ::window/buffername buffername)
        (swap! editor update ::windows coreutil/doto-first assoc ::window/buffername buffername))))
...
(defn other-window
  ...
  (let [buffername (-> @editor ::windows second (window/get-buffername))]
    (swap! editor update ::windows coreutil/rotate)
    (swap! editor update ::buffers coreutil/bump ::buffer/name buffername)))
...
(defn remove-buffer
  [buffername]
  (swap! editor update ::buffers coreutil/remove-item ::buffer/name buffername))
